...
# Uses v3 figures from the CarbonAPI at https://sustainablewebdesign.org/estimating-digital-emissions-version-3
...
my $PERCENTAGE_OF_ENERGY_IN_DATACENTER = 0.22;	# V3=0.15, V2=0.1008
my $PERCENTAGE_OF_ENERGY_IN_TRANSMISSION_AND_END_USER = 0.78;	# V3=0.85, V2=0.8992
...
#V3	# Calculate the statistics as we need the co2 emissions
...
#	$co2 = ($results{'greenweb'}{'green'} ? $statistics->{'co2'}{'renewable'}{'grams'} : $statistics->{'co2'}{'grid'}{'grams'});
	...
	$OPDC = 0.82;			# Operational Emissions Data Centers
	$GreenHostingFactor = ($results{'greenweb'}{'green'} ? 1 : 0); # The portion of hosting services powered by renewable or zero-carbon energy, between 0 and 1
	$EMDC = 0.18;			# Embodied Emissions Data Centers
	$OPN = 0.82;			# Operational Emissions Networks
	$EMN = 0.18;			# Embodied Emissions Networks
	$OPUD = 0.49;			# Operational Emissions User Devices
	$EMUD = 0.51;			# Embodied Emissions User Devices
	$NewVisitorRatio = 1;	# The portion of first time visitors to a web page, between 0 and 1
	$ReturnVisitorRatio = 1;# The portion of returning visitors to a web page, between 0 and 1
	$DataCacheRatio = 1;	# The portion of data that is loaded from cache for returning visitors, between 0 and 1

	$co2 = ((($OPDC * (1 - $GreenHostingFactor) + $EMDC) + ($OPN + $EMN) + ($OPUD + $EMUD)) * $NewVisitorRatio) + ([(OPDC × (1 - Green Hosting Factor) + EMDC) + (OPN + EMN) + (OPUD + EMUD)] × Return Visitor Ratio × (1 - Data Cache Ratio));
